...
class CustomEnv(gym.Env):
    def __init__(self, agent_name='SimpleFlight',__multirotor_client=None):
        ...
        self.drone_process = __multirotor_client.takeoffAsync(timeout_sec = 1, vehicle_name = agent_name)
        ...
        self.pose = self.__multirotor_client.simGetVehiclePose(vehicle_name = self.agent_name)
        self.kinematics_estimated = self.__multirotor_client.getMultirotorState(vehicle_name = self.agent_name).kinematics_estimated
    ...
    def reset(self, seed=None, options=None):
        ...
        #self.__multirotor_client.hoverAsync(vehicle_name = self.agent_name).join()
        #self.__multirotor_client.moveByVelocityAsync(0.01, -0.01, -0.01, 0.5,vehicle_name = self.agent_name).join()
    ...
    def step(self, action):
        ...
        self.__multirotor_client.simPause(True)
        self.kinematics_estimated = self.__multirotor_client.getMultirotorState(vehicle_name = self.agent_name).kinematics_estimated
        ...
        self.__multirotor_client.simPause(False)
    ...
    def __get_image(self,image_type = "rgb"):
        image_response = self.__multirotor_client.simGetImages([airsim.ImageRequest(0, airsim.ImageType.DepthVis, False, False)],vehicle_name = self.agent_name)[0]
    ...
    def __get_multirotor_orientation(self):
        orientation = self.__multirotor_client.simGetVehiclePose(vehicle_name = self.agent_name).orientation
    ...
    def __set_agent_location(self, locations):
        pose = self.__multirotor_client.simGetVehiclePose(vehicle_name = self.agent_name)           
        pose.position.x_val = locations[0]
        pose.position.y_val = locations[1]
        pose.position.z_val = locations[2]
        
        self.__multirotor_client.simSetVehiclePose(pose=pose, ignore_collision=True,vehicle_name = self.agent_name)

    def _do_action(self, action):
        quad_offset = self.interpret_action(action)
        quad_vel = self.kinematics_estimated.linear_velocity
        self.drone_process = self.__multirotor_client.moveByVelocityAsync(
    ...
    def __compute_reward(self):
        ...
        if self.__multirotor_client.simGetCollisionInfo(vehicle_name = self.agent_name).has_collided:
        ...
        #multirotor_state = self.__multirotor_client.getMultirotorState(vehicle_name = self.agent_name)
        #orientation = multirotor_state.kinematics_estimated.orientation
        
        quad_pt = self.agent_current_location

        quad_vel_orgin = self.kinematics_estimated.linear_velocity
