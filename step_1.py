...
class CustomEnv(gym.Env):
    def __init__(self, vehicle_name, num_history=3):
        ...
        self.client.takeoffAsync(timeout_sec = 1,vehicle_name = self.vehicle_name).join()
        self.client.moveToZAsync(-10,2,vehicle_name = self.vehicle_name).join()
    ...
    def reset(self, seed=None, options=None):
        ...
        self.client.moveByVelocityAsync(
            ...
            0,
            0,
            0.1,vehicle_name = self.vehicle_name       
        ).join()
        self.total_reward = 0
        self.platform_detected = False
    ...
    def step(self, action):
        ...
        vz = -float(action[2])*max_speed
        
        #print(f'\rvx: {vx}, vy: {vy}, vz: {vz}', end='')
        
        self.client.moveByVelocityBodyFrameAsync(vx, vy, vz, 1,vehicle_name = self.vehicle_name)
        '''
        #print(action)
        ...
        self.client.moveByVelocityAsync(
            ...
            1,vehicle_name = self.vehicle_name       
        )
        '''
    ...
    def _compute_reward(self):
        ...
        collide_info = self.client.simGetCollisionInfo(vehicle_name = self.vehicle_name)
        # Check for collision first
        if collide_info.has_collided:
            # Set values for this agent and continue to the next agent
            #print("Collision detected")
            if "BP_NPC" in collide_info.object_name:
                reward = +50
                terminations = True
                truncations = False
        ...
        if detected_pad:
            ...
            reward += -1  # -1 if the pad is not detected
        '''
        ...
        if detected_pad:
            ...
            reward += np.exp(-distance_from_center)

            # Apply additional reward based on bounding box size (indicating closeness)
            reward += 1 / (1 + 0.5 - box_area)  # As the drone gets closer (larger box), give more reward
        '''
        drone_position = self.client.getMultirotorState().kinematics_estimated.position.to_numpy_array()
        distance_to_origin = np.linalg.norm(drone_position)
        
        # Set the steepness factor
        alpha = 0.5

        # Calculate exponential reward
        reward += 2*np.exp(-alpha * distance_to_origin)
        ...
        # Check for loss of helipad visibility for x consecutive frames
        if self.consecutive_no_detection >= 15:
            terminations = True
